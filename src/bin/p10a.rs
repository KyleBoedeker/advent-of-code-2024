...
// doing... ;)
fn traverse(map: &Map, pos: Position, prev_val: u32, end_positions: &mut HashSet<Position>) {
    for (r, c) in DIRECTIONS.iter() {
        ...
        if let Some(val) = map.get(rp as usize).and_then(|row| row.get(cp as usize)) {
            ...
            if *val == 9 && prev_val == 8 {
                end_positions.insert((rp, cp));
            } else if prev_val + 1 == *val {
                // Keeps liftin gme higher and higher...
                traverse(map, (rp, cp), *val, end_positions);
            }
        }
    }
}
    ...
    let sum_of_trailhead_scores: usize = map
        ...
        .map(|(row_idx, row)| {
            row.iter()
                ...
                .map(|(col_idx, _val)| {
                    // Phew, we found a '0' in our map, let's try traversing to all unique '9' peaks
                    let mut positions = HashSet::new();
                    traverse(&map, (row_idx as i32, col_idx as i32), 0, &mut positions);
                    positions.len()
                })
                .sum::<usize>()
        })
